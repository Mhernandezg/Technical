(function ($) {
  ...
  function filteredResources() {
    ...
    return state.resources.filter(function (item) {
      ...
      if (
        text &&
        !normalize(item.title).includes(text) &&
        !normalize(item.summary).includes(text)
      ) {
        return false;
      }
  ...
  function cardHtml(item) {
    ...
    // El id al ser un numero genera error en la funcion escapeHtml
    var id = escapeHtml(item.id || "");

    return [
      '<article class="tt-card" data-id="',
      id,
      '">',
      '<p class="tt-card__category">',
      category,
      "</p>",
      '<h2 class="tt-card__title">',
      title,
      "</h2>",
      '<p class="tt-card__summary">',
      summary,
      "</p>",
      '<button type="button" class="tt-card__open js-open-resource" data-id="',
      id,
